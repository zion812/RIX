...
exports.demoWebhookEndpoint = exports.processDemoGooglePay = exports.processDemoUPIPayment = exports.processDemoPayment = exports.createDemoPaymentOrder = void 0;
 ...
 */
exports.createDemoPaymentOrder = functions
    .region(REGION)
    .https
 ...
 */
exports.processDemoPayment = functions
    .region(REGION)
    .https
    .onCall(async (data, context) => {
    if (!context.auth) {
 ...
 */
exports.processDemoUPIPayment = functions
    .region(REGION)
    .https
    .onCall(async (data, context) => {
    if (!context.auth) {
 ...
 */
exports.processDemoGooglePay = functions
    .region(REGION)
    .https
    .onCall(async (data, context) => {
    if (!context.auth) {
 ...
 */
exports.demoWebhookEndpoint = functions
    .region(REGION)
    .https
 ...
 * Helper functions
 */
async function processDemoSuccess(orderId, orderData, paymentMethod) {
...
async function processDemoFailure(orderId, orderData, paymentMethod) {
    const failureReasons = [
        'Insufficient funds',
        'Card declined',
        'Network timeout',
...
function shouldSimulateFailure(orderData, paymentMethod) {
...
function getRealisticDelay(paymentMethod) {
...
function generateDemoQRCode(orderId, amount) {
...
async function updateDemoOrderStatus(orderId, status, additionalData) {
    await db.collection('demo_orders').doc(orderId).update(Object.assign({ status: status, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, additionalData));
}
async function triggerDemoWebhook(orderId, event, paymentData) {
    ...
    setTimeout(async () => {
        try {
            ...
            await db.collection('demo_webhooks').add(Object.assign(Object.assign({}, webhookData), { createdAt: admin.firestore.FieldValue.serverTimestamp() }));
...
async function handleDemoPaymentCaptured(orderId, paymentData) {
...
async function handleDemoPaymentFailed(orderId, paymentData) {
...
async function logDemoTransaction(data) {
    await db.collection('demo_transaction_logs').add(Object.assign(Object.assign({}, data), { timestamp: admin.firestore.FieldValue.serverTimestamp() }));
}
//# sourceMappingURL=demoPaymentGateway.js.map