...
exports.processScheduledNotifications = exports.sendNotification = exports.NotificationPriority = exports.NotificationCategory = void 0;
...
(function (NotificationCategory) {
    ...
    NotificationCategory["COMMUNICATION"] = "communication";
    NotificationCategory["BREEDING"] = "breeding";
 ...
 */
async function processNotification(payload, senderId) {
 ...
 */
async function sendImmediateNotification(notificationId, payload) {
    ...
    try {
        ...
        if (payload.targetUsers && payload.targetUsers.length > 0) {
            ...
            if (tokens.length > 0) {
                response = await messaging.sendMulticast(Object.assign({ tokens: tokens }, fcmPayload));
        ...
        if (payload.targetTopics && payload.targetTopics.length > 0) {
            ...
            for (const topic of payload.targetTopics) {
                await messaging.send(Object.assign({ topic: topic }, fcmPayload));
            }
        }
 ...
 */
async function buildFCMPayload(payload) {
    const fcmPayload = {
        notification: {
            title: payload.title,
            body: payload.body
        },
        data: Object.assign({ category: payload.category, priority: payload.priority, notificationId: `notif_${Date.now()}`, deepLink: payload.deepLink || '' }, payload.data),
        android: {
 ...
 */
async function updateDeliveryStats(notificationId, successCount, failureCount) {
    await db.collection('notifications').doc(notificationId).update({
 ...
 */
async function scheduleNotification(notificationId, scheduledTime) {
 ...
 */
function getNotificationChannel(category) {
    switch (category) {
...
exports.processScheduledNotifications = functions
    ...
    try {
        ...
        for (const notification of notifications) {
            try {
                await sendImmediateNotification(notification.id, notification);
                // Mark as sent
