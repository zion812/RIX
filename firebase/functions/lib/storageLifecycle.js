...
exports.weeklyStorageOptimization = exports.dailyStorageCleanup = void 0;
 ...
 */
exports.dailyStorageCleanup = functions
    .region('asia-south1')
    .pubsub
 ...
 */
exports.weeklyStorageOptimization = functions
    .region('asia-south1')
    .pubsub
 ...
 */
async function moveToNearlineStorage() {
 ...
 */
async function moveToColdlineStorage() {
    const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
    let movedCount = 0;
    let savedCost = 0;
    const [files] = await bucket.getFiles({
 ...
 */
async function optimizeUserStorageQuotas() {
    let usersNotified = 0;
 ...
 * Helper functions
 */
async function isMarketplaceItemSold(filePath) {
    var _a;
    ...
    try {
        ...
        return listingDoc.exists && ((_a = listingDoc.data()) === null || _a === void 0 ? void 0 : _a.listingStatus) === 'SOLD';
    }
    catch (error) {
        return false;
    }
...
function getStorageQuota(userTier) {
    switch (userTier) {
        case 'GENERAL':
...
async function sendStorageQuotaNotification(userId, usage, quota) {
...
function calculateEstimatedCost(distribution) {
    ...
    for (const [storageClass, data] of Object.entries(distribution)) {
        ...
        totalCost += sizeGB * costs[storageClass];
    }
    return totalCost;
}
// Additional optimization functions would be implemented here...
async function analyzeStorageUsage() { return {}; }
async function optimizeImageFormats() { return {}; }
async function consolidateUserData() { return {}; }
async function archiveOldTransferData() { return {}; }
async function generateCostReport() { return {}; }
//# sourceMappingURL=storageLifecycle.js.map